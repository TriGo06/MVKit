...
mod functions {
    ...
    /// Simulate the Kuramoto coupled-oscillator model.
    ///
    /// See `python/mvkit/__init__.py` for the user-facing docstring.
    #[pyfunction]
    #[pyo3(signature = (
    ...
    pub(super) fn simulate_kuramoto<'py>(
        ...
        if x0_view.ncols() != 1 {
            return Err(pyo3::exceptions::PyValueError::new_err(format!(
                "x0 has {} columns, expected 1 (Kuramoto state is scalar phase)",
                x0_view.ncols()
            )));
        }
        if omegas_view.len() != x0_view.nrows() {
            return Err(pyo3::exceptions::PyValueError::new_err(format!(
                "omegas has length {} but x0 has {} rows; they must match",
                omegas_view.len(),
                x0_view.nrows()
            )));
        }
        ...
        if !coupling_k.is_finite() {
            return Err(pyo3::exceptions::PyValueError::new_err(
                "coupling_k must be finite",
            ));
        }
        if !sigma.is_finite() || sigma < 0.0 {
            return Err(pyo3::exceptions::PyValueError::new_err(

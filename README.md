...
Roadmap. The non-LQ MFG pipeline is end-to-end and supports both periodic and Neumann boundaries: `mvkit.mfg.solve_hjb` (backward HJB), `mvkit.mfg.solve_fokker_planck` (forward FP), and `mvkit.mfg.solve_mfg` (Picard / Fictitious Play coupling, validated against the closed-form LQ-MFG in both symmetric and asymmetric settings). Vector LQ-MFG with matrix Riccati is also available via `mvkit.mfg.solve_lq_mfg_vector` for multi-dimensional state. Next: multi-dimensional grid solver and non-quadratic Hamiltonians.

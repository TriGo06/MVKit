...
Roadmap. Phases 1 and 2 of the non-LQ MFG plan are in: `mvkit.mfg.solve_hjb` (backward HJB) and `mvkit.mfg.solve_fokker_planck` (forward FP). Phase 3 will combine the two via Picard / Fictitious Play iteration into a full `solve_mfg(problem, ...)` entry point and validate against the closed-form LQ-MFG. Beyond that: vector LQ-MFG with matrix Riccati, Neumann BC, and multi-dimensional state.

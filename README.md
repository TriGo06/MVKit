...
`mvkit.mfg` is a new sub-module dedicated to numerical MFG. The first release ships the scalar linear-quadratic case, where the HJB ansatz $u(t, x) = \tfrac{1}{2} P(t) x^2 + Q(t) x + R(t)$ reduces the problem to scalar ODEs (Riccati for $P$, linear ODE for the variance), so the equilibrium is known in closed form. The solver uses a generic Picard iteration on the mean trajectory designed to extend to non-LQ MFG once a full HJB grid solver is in place; on LQ it converges in a handful of iterations and the closed-form variance gives a quantitative benchmark.
...
The closed-form construction follows Carmona and Delarue (2018), *Probabilistic Theory of Mean Field Games with Applications I*, Section 3.5. See `examples/mfg_lq_demo.py` for a runnable three-panel figure showing $P(t)$, the analytical-vs-empirical variance trajectory, and a sample of particle trajectories with the equilibrium mean overlaid.

Roadmap. v0.2 will add a generic HJB solver on a grid (semi-Lagrangian or upwind FD) for non-LQ MFG, fictitious play with historical averaging as an alternative to Picard, and multi-dimensional state.
